...
export const CircleMarker = L.CircleMarker.extend({
  ...
  showPopup(content) {
    ...
    if (typeof popup === 'string') {
      // Template string
      this.bindPopup(
        L.Util.template(popup, this._feature.properties)
      ).openPopup();
    } else if (popup instanceof Function) {
      // Function returning string
      popup(
        this._feature,
        L.bind(function show(response) {
...
export function circleMarker(feature, options) {
